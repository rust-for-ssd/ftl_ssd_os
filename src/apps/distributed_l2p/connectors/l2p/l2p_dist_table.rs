...
pub struct L2PDistributionTable<A: Allocator + 'static, const NUM_PIPES: usize> {
    last_picked: u8,
    table: BTreeMap<LogicalAddr, u8, &'static A>,
    pipe_names: [&'static CStr; NUM_PIPES],
}

impl<A: Allocator + 'static, const NUM_PIPES: usize> L2PDistributionTable<A, NUM_PIPES> {
    pub fn new(alloc: &'static A, pipe_names: [&'static CStr; NUM_PIPES]) -> Self {
        Self {
            last_picked: 0,
    ...
    pub fn pick_table_idx(&mut self) -> u8 {
        ...
        self.last_picked = (self.last_picked + 1) % NUM_PIPES as u8;
        res
    }
    ...
    pub fn prepare_for_benchmark(&mut self, n_reqs: usize) {
        for i in 0..n_reqs {
            self.table.insert(i as u32, (i % NUM_PIPES) as u8);
        }
    }
}
