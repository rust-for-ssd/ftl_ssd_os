...
struct pipeline *conn1_fn (struct lring_entry *entry)
{
    ...
    if (!pipe1)
        pipe1 = (struct pipeline *) ssd_os_get_connection("round_trip_conn1", "round_trip_pipe1");
...
struct pipeline *conn2_fn (struct lring_entry *entry)
{
    ...
    if (!pipe2)
        pipe2 = (struct pipeline *) ssd_os_get_connection("round_trip_conn2", "round_trip_pipe2");
