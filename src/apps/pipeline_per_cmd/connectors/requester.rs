...
fn pipe_start(entry: *mut lring_entry) -> *mut pipeline {
    ...
    // 
    // 
    let mut pipe : *mut pipeline = core::ptr::null_mut();
    ...
    unsafe {
        if AMOUNT_IN_LRING < RING_SIZE as i32 {
            if let Some(idx) = get_free_message_index() {
                ...
                match (*msg_ptr).cmd {
                    CommandType::READ => pipe = ssd_os_get_connection(c"requester1", c"read"),
                    CommandType::WRITE => pipe = ssd_os_get_connection(c"requester1", c"write"),
                    CommandType::ERASE => pipe = core::ptr::null_mut(),
                }
